...
pub async fn swap_indexes(
    ...
    params: ValidatedJson<Vec<SwapIndexesPayload>, IndexesDeserError>,
    req: HttpRequest,
...
#[derive(Debug)]
pub struct IndexesDeserError {
    error: String,
    code: Code,
}

impl std::fmt::Display for IndexesDeserError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.error)
    }
}

impl std::error::Error for IndexesDeserError {}
impl ErrorCode for IndexesDeserError {
    fn error_code(&self) -> Code {
        self.code
    }
}

impl deserr::MergeWithError<IndexesDeserError> for IndexesDeserError {
    fn merge(
        _self_: Option<Self>,
        other: IndexesDeserError,
...
impl deserr::DeserializeError for IndexesDeserError {
    fn error<V: IntoValue>(
        ...
        Err(IndexesDeserError { error, code })
    }
}
