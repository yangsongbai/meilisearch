...
mod tests {
    ...
    impl IndexScheduler {
        ...
        pub fn maybe_fail(&self, location: FailureLocation) -> Result<()> {
            if self
                .planned_failures
                .contains(&(*self.run_loop_iteration.read().unwrap(), location))
            {
                match location {
    ...
    impl IndexSchedulerHandle {
        ...
        fn wait_till(&self, breakpoint: Breakpoint) {
            self.test_breakpoint_rcv
                .iter()
                .find(|b| *b == (breakpoint, false));
        }
    }
    ...
    fn fail_in_update_task_after_process_batch_success_for_document_addition() {
        ...
        let (index_scheduler, handle) = IndexScheduler::test(
            ...
            vec![(
                1,
                FailureLocation::UpdatingTaskAfterProcessBatchSuccess { task_uid: 0 },
            )],
        );
