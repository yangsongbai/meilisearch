...
impl IndexScheduler {
    ...
    pub fn assert_internally_consistent(&self) {
        ...
        for task in self.all_tasks.iter(&rtxn).unwrap() {
            ...
            match details {
                Some(details) => match details {
                    Details::DocumentAddition {
                        ...
                        assert_eq!(kind.as_kind(), Kind::DocumentImport);
                    ...
                    Details::Settings { settings: _ } => {
                        assert_eq!(kind.as_kind(), Kind::Settings);
                    }
                    ...
                    Details::ClearAll { deleted_documents } => {
                        assert!(matches!(
                            kind.as_kind(),
                            Kind::DocumentClear | Kind::IndexDeletion
                        ));
            ...
            match kind {
                KindWithContent::DocumentImport { content_file, .. } => match status {
